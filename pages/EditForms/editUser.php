...
<div class="card">
  ...
  <div class="card-body">
    ...
    <div class="col-sm-10">
      <input type="text" readonly class="form-control" id="name" name="iduser" value= <?php echo $_GET['id']?> placeholder= <?php echo $_GET['id']?>>
    </div>
  </div>
...
require_once ("../../controller/validations.php");
  ...
  $iduser=$_POST['iduser']?sanitizeInput($_POST['iduser']):''; 

  if(empty($_POST['name'])||empty($_POST['password'])||empty($_POST['role'])){
    header("Location:./editUser.php?id=".$_SESSION['id']."&error=invalidbox");
    exit();
  }

  if( !alphaNumeric($name) || strlen($name) > 30 || $name == "Enter a name"){
    header("Location:./editUser.php?id=".$_SESSION['id']."&error=validname");
    exit();
  }
    ...
    $db->updateUser($name,$passwordHashed,$role,$iduser);
