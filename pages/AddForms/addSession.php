...
if ($_SERVER["REQUEST_METHOD"] == "POST") { 
  ...
  if(empty($name)||empty($numberallowed)){
    header("Location:./addSession.php?&error=invalidbox");
    exit();
  }

  else if(!alphaNumeric($name) || strlen($name) > 30 || $name == "Enter a name"){
    header("Location:./addSession.php?&error=validname");
    exit();
  }

  else if(!IntegerNum($numberallowed)){
    header("Location:./addSession.php?&error=validnumber");
    exit();  
  }

  else if(!validateDate($startdate,$enddate)){
    header("Location:./addSession.php?&error=validdate");
    exit();  
  }

else{ 
  $db->insertSession($name,$numberallowed,$event,$startdate,$enddate);
  if($_SESSION['role']==1){
