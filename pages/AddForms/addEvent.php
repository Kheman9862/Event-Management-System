...
<div class="card">
  ...
  <div class="card-body">
    ...
    <p class="card-text">Please add an event to the table</p>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $datestart=$_POST['datestart']?sanitizeInput($_POST['datestart']):'';
  $dateend=$_POST['dateend']?sanitizeInput($_POST['dateend']):'';
  $numberallowed = $_POST['numberallowed']?sanitizeInput($_POST['numberallowed']):'';
  $venue= $_POST['venue'];
  $manager= $_POST['manager'];


  if(empty($name)||empty($numberallowed)){
    header("Location:./addEvent.php?&error=invalidbox");
    exit();
  }

  else if(!alphaNumeric($name) || strlen($name) > 30 || $name == "Enter a name"){
    header("Location:./addEvent.php?&error=validname");
    exit();
  }

  else if(!IntegerNum($numberallowed)){
    header("Location:./addEvent.php?&error=validnumber");
    exit();  
  }

  else if(!validateDate($datestart,$dateend)){
    header("Location:./addEvent.php?&error=validdate");
    exit();  
  }
  ...
  $db->insertEvent($name,$datestart,$dateend,$numberallowed,$venue,$manager);
